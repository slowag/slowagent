...
def train(args, envs, actor, qf1, qf2, qf1_target, qf2_target, q_optimizer, actor_optimizer, a_optimizer, rb, writer, device, alpha, log_alpha, target_entropy):
    ...
    if args.eval_model:
        ...
        episodic_returns = evaluate_sac(actor, args, args.device)
        ...
        episodic_returns = evaluate_sac(actor, args, args.device, greedy=True)
...
def train_delayed_buffer_hiddens(args, envs, actor, qf1, qf2, qf1_target, qf2_target, q_optimizer, actor_optimizer, a_optimizer, rb, writer, device, alpha, log_alpha, target_entropy):
    start_time = time.time()
    # TRY NOT TO MODIFY: start the game
    obs, _ = envs.reset(seed=args.seed)
    actor_updates = 0
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            with torch.no_grad():
                actor.eval()
                next_state_actions, next_state_log_pi, _ = actor.learn_action(data.next_observations)
                actor.train()
            ...
            if global_step % args.policy_frequency == 0:  # TD 3 Delayed update support
                for _ in range(
                    ...
                    actor_updates += 1
                    actor_optimizer.zero_grad()
                    pi, log_pi, _ = actor.learn_action(data.observations)
                    ...
                    if args.autotune:
                        with torch.no_grad():
                            actor.eval()
                            _, log_pi, _ = actor.learn_action(data.observations)
                            actor.train()
                        alpha_loss = (-log_alpha.exp() * (log_pi + target_entropy)).mean()
...
def train_delayed_efficient(args, envs, actor, qf1, qf2, qf1_target, qf2_target, q_optimizer, actor_optimizer,
                            a_optimizer, rb, writer, device, alpha, log_alpha, target_entropy):
    start_time = time.time()
    # TRY NOT TO MODIFY: start the game
    obs, _ = envs.reset(seed=args.seed)
    actor_updates = 0
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            if batch_hidden_activations is None:
                rb.reset_indices(args.batch_size)

            data = rb.sample_next()

            if batch_hidden_activations is None:
                batch_hidden_activations = actor.get_activations(data.observations)
                for _ in range(2*(args.N_hidden_layers + 2)):
                    _, _, _, batch_hidden_activations = actor.get_action(data.observations, batch_hidden_activations)

            # actor update
            actor_updates += 1
            actor_optimizer.zero_grad()
            pi, log_pi, _, batch_hidden_activations = actor.get_action(data.observations, batch_hidden_activations)
            qf1_pi = qf1(data.observations, pi)
            qf2_pi = qf2(data.observations, pi)
            min_qf_pi = torch.min(qf1_pi, qf2_pi)
            actor_loss = ((alpha * log_pi) - min_qf_pi).mean()
            ...
            with torch.no_grad():
                actor.eval()
                next_state_actions, next_state_log_pi, _, _ = actor.get_action(data.next_observations, batch_hidden_activations)
                actor.train()
                qf1_next_target = qf1_target(data.next_observations, next_state_actions)
                qf2_next_target = qf2_target(data.next_observations, next_state_actions)
                min_qf_next_target = torch.min(qf1_next_target, qf2_next_target) - alpha * next_state_log_pi
            ...
            if global_step % 64 == 0:
                for bha in batch_hidden_activations:
                    if not isinstance(bha, tuple):
                        bha.detach_()
                    else:
                        for h in bha:
                            if h is not None:
